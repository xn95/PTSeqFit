...
def data2slices(data_2theta, theta_peak_guess, theta_variance):
	sliced_data = []
...
def gauss(x, amp, cen, sigma, shift):
	"""Gaussian lineshape."""
	return amp * np.exp(-(x-cen)**2 / (2.*sigma**2)) + shift

def split_model(i, region, gaussian_params, peaks):
...
def fit_function(parameters, SG_num, ttheta_max, wavelength, data_file, theta_variance, num_peaks):
	#this function will break down if a peak crosses the ttheta_max boundary, as the number of peaks generated by the indexer
    #will be different from the number of gaussian parameters supplied....
    #an idea is to not use ttheta_max to generate the peak list but to rather generate a full peak list and to cut this down
    #to the number of peaks specifed by num_peaks
    #the old code is indicated and commented out:
    
    
	#get the data into list of [x,y]s
	data_2theta = strip_zeros(data_file)
    ...
    #OLD CODE:
# 	indexing_info = xu.simpack.PowderDiffraction(material, tt_cutoff = ttheta_max, enable_simulation = False, wl = wavelength).data
# 	peak_list = []
...
def SG_lattice_params2material(SG_num, in_lattice_params):
    #in_lattice_params = [float(i) for i in in_lattice_params]
...
class Main_window(wx.Frame):
    ...
    def open_calibration(self, event):#open a .pomi
    ...
    def datafile_plot(self, event):#quick plot of raw data
        ...
        loaded_data = strip_zeros(filename)#[[x,y],]
    ...
    #while it would make more sense to make a static list of [a,b,c,alpha,beta,gamma]
    #xrayutilities doesn't play this way
        if self.SG_num <= 230 and self.SG_num > 194:#cubic
    ...
    def SG_input(self, event):  # stops the user from doing anything too stupid
    ...
    def theta_variance2datapoints(self, event): #estimates number of data points which makes up the 2theta variance entered in the GUI
        ...
        data_frame = strip_zeros(self.loaded_datafiles[0])#load in 1st dataset [[x,y],[x,y]....]
    ...
    #there appears to be an issue if a peak crosses a masked region
    #it may be worth doing quick error checking:
        #check if the parameters out are the same as the parameters in 
        #if so.... do an error check function
        #check if a peak position lines between data with zeros
        #if so ignore this peak?
    #make a backup file:
    ...
    #into a list of [scale, scale,... sigma, sigma,...  shift, shift,... per number of peaks]
        ...
        for frame in framelist:
            #IF YOU LIST CONCATINATE A ONE ELEMENT LIST IT JUST PAIRWISE ADDS THE VALUE OF THE ONE ELEMENT LIST TO THE ELEMENTS
            #OF THE OTHER LIST
            #FUCK THIS PYTHON BEHAVIOUR IN PARTICULAR
            ...
            if LS_params == list(LS_out.x):
                self.log.WriteText("Refinement returned input values\n")
                # self.log.WriteText("Introducing noise and reattempting...\n")
                ...
                #the random noise was the wrong approach
                # #refinement issue
                # random_list = np.random.rand(len(LS_params))
                # LS_noised = []
                # for value in LS_params:
                #     index = LS_params.index(value)
                #     LS_noised.append(value+(0.05*value * random_list[index]))#introduces up to 5% variation in each parameter
            ...
            #I think this behaviour could be solved by opening a new thread
            
    def do_seq_threaded(self,event):
        t = Thread(target=self.do_seq_fit, args=[event])
        t.run()
        #theres probably more that needs to be done to avoid hanging the GUI
...
class EoSDialog(wx.Dialog):
    ...
    def __init__(self, parent, *args, **kwds):
        ...
        if self.loaded_calibrant in [i.get("name") for i in EoS.dictionaries]:
            ...
            #write code that allows editing and saving of these EoS params to a file
            #and code that allows for importing from such a file
            #finally write code that does the PVT calc 
